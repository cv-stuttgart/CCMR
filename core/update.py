...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, correlation_depth, hidden_dim=128, input_dim=256, scale=8, num_heads=8, depth=1, mlp_ratio=1, num_scales=4):
        ...
        self.aggregator = nn.ModuleList([XCiT(embed_dim=128, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, separate=True)])
        for i in range(num_scales - 1):
            self.aggregator.extend([XCiT(embed_dim=128, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, separate=True)])
