...
class XCABlock(nn.Module):
    def __init__(self, dim, num_heads=8, mlp_ratio=1, qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 separate=False, eta=None):
        ...
        if separate:
            self.attn = XCA_separate(
                dim, qkv_bias=qkv_bias, attn_drop=attn_drop,
                proj_drop=drop
            )
        else:
            self.attn = XCA(
    ...
    def forward(self, x, H, W, x_v = None):
        if self.separate:
            x = x + self.drop_path(self.gamma1 * self.attn(x_qk=self.norm1(x), x_v=self.norm1(x_v)))
        else:
...
class XCiT(nn.Module):
    ...
    def __init__(self, embed_dim=128,
                 depth=1, num_heads=8, mlp_ratio=1, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.05, norm_layer=None, use_pos=True, eta=1.0, separate=False):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            XCABlock(
                ...
                norm_layer=norm_layer, eta=eta, separate=self.separate)
    ...
    def forward(self, x, x_v=None):
        ...
        pos_encoding = self.pos_embeder(B, H, W).reshape(B, -1, x.shape[1]).permute(0, 2, 1) 
        if self.separate:
